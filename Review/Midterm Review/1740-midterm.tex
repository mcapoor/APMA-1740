...
\begin{multicols}{2}
    ...
    Let $X \in \R^d$ be a RV with classes $Y \in \{1, \dots, c\}$. Define $\pi_i = \P(Y = i)$ and $f_i(x)$ the class conditioned density.
    ...
    \begin{proposition}
        ...
        \tbf{Note:} In the case $t = \frac{\pi_1}{\pi_2}$, then NP is equivalent to Bayes'.
    \end{proposition}

    \tbf{Detection Rate:} $\P(h(X) = 2 \; | \; Y  =2)$

    \tbf{False Alarm Rate:} $\P(h(X) = 2 \; | \; Y = 1)$

    \begin{proposition}
        \textbf{Theorem:} Fix $t \in (0, \infty)$ and choose any classifier $h$. If $\text{FAR}(h) \leq \text{FAR}(h_t)$, then $\text{DR}(h) \leq \text{DR}(h_t)$
    \end{proposition}

    \subsection*{Generative Classifiers}

    \emph{Motivation:} From Bayes,
    \[h*(x) = \argmax_{x \in \{1, \dots, s\}} \P(Y = c \; | \; X = x) = \argmax_c \frac{\pi_c f_c(x)}{\P(X = x)}\]
    so we can estimate $\hat \pi_c \approx \pi_c$ and then it suffices to estimate $f_c(x)$.

    \tbf{Naive Bayes:} Assume $f_c(x_1, \dots, x_d) = \prod_{j=1}^d f_{c, j}(x_j)$. Then instead of needing to find $(f_c)^s$ with $f_c: \R^d \to \R$, it suffices to find $f_{c, j}: \R \to \R$.

    \tbf{Quadratic Discriminant Analysis:} Assume $f_c(x) \sim \Nc(\mu_c, \Sigma_c)$. Then
    \[f_c(x, \mu_c, \Sigma_c) = \frac{1}{\sqrt{(2\pi)^d \abs{\Sigma}}} \exp\left(-\frac{(x -\mu_c)\Sigma_c^{-1}(x - \mu_c)}{2}\right)\]
    for $x, \mu_c in \R^d$ and $\Sigma_c \in \R^{d\times d}$.

    \tbf{Linear Discriminant Analysis:} Assume $f_c(x) \sim \Nc(\mu_c, \Sigma)$.

    \subsection*{Discriminative Classifiers:}
    \emph{Motivation:}
    \[h*(x) = \argmax_{x \in \{1, \dots, s\}} \P(Y = c \; | \; X = x) = \argmax_c r_c(x)\]

    \tbf{Linear Regression:} Assume $s = 2$ so $r_1(x) + r_2(x) = 1$. Then
    ...
    \tbf{Softmax:} Assume $s > 2$. Then model
